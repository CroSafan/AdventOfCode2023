...
public class Day9A {
	...
	private int predictValue(String line) {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			if (rows.get(i + 1).stream().distinct().toList().get(0) == 0 && rows.get(i + 1).isEmpty() || rows.get(i + 1).stream().allMatch(rows.get(i + 1).get(0)::equals)) {
				break;
			}
		}
		int result=0;
		for(int i = rows.size()-1;i>=0;i--) {
			...
			result=rows.get(i).get(rows.get(i).size()-1)+result;
